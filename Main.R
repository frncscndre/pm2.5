...
Extraccion_reflectancias<-function(recalcular=FALSE){
  setwd(directorio_instalacion)
  setwd('./Recursos/Estaciones PM Vectorial')
  shape_pm2.5<-shapefile("Estaciones, sistema coordenadas proyecto.shp")
  ...
  for (i in 1:length(carpetas)){
    ...
    if(!any(imagenes_extraidas)|recalcular){
      ...
      extract_TOA1<-extract(TOA_B1,shape_pm)
      extract_TOA2<-extract(TOA_B2,shape_pm)
      extract_TOA3<-extract(TOA_B3,shape_pm)
      extract_TOA4<-extract(TOA_B4,shape_pm)
      extract_TOA5<-extract(TOA_B5,shape_pm)
      extract_TOA6<-extract(TOA_B6,shape_pm)
      extract_TOA7<-extract(TOA_B7,shape_pm)
      extract_TOA8<-extract(ND_B8,shape_pm)
      extract_TOA9<-extract(TOA_B9,shape_pm)
      
      extract_RA_B1<-extract(TOA_B1,shape_pm)
      extract_RA_B2<-extract(TOA_B2,shape_pm)
      extract_RA_B3<-extract(TOA_B3,shape_pm)
      extract_RA_B4<-extract(TOA_B4,shape_pm)
      extract_RA_B5<-extract(TOA_B5,shape_pm)
      extract_RA_B6<-extract(TOA_B6,shape_pm)
      extract_RA_B7<-extract(TOA_B7,shape_pm)
      extract_TOA_B10<-extract(TOA_B10,shape_pm)
      extract_TOA_B11<-extract(TOA_B11,shape_pm)
      extract_PixelQA<-extract(PixelQA,shape_pm)
      extract_aerosolQA<-extract(aerosol_QA,shape_pm)
      df_extraciones<-data.frame(nombre_fecha,shape_pm$cve_estac,extract_SR1,extract_SR2,extract_SR3,extract_SR4,extract_SR5,extract_SR6,extract_SR7,
